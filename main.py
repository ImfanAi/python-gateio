    ...
    try:
        ...
        if take_profit > 0:
            ...
            tp_order = gate_api.FuturesOrder(
                contract=contract,
                size=-size,  # Opposite of the original position size
...
def close_futures_position(symbol):
    ...
    try:
        ...
        positions = futures_api.list_futures_positions('usdt')
        for position in positions:
            if position.contract == symbol and position.size != 0:
                # Close the position
                close_order = gate_api.FuturesOrder(
                    contract=symbol,
                    size=-position.size,  # Close the entire position
                    price=str(position.last),  # Use last price for closing
                    tif='gtc',
                    close=True
                )
        ...
        open_orders = futures_api.list_futures_orders('usdt', contract=symbol)
...
def main():
    ...
    open_parser.add_argument('symbol', type=str, help='Token symbol (e.g., DOGEUSDT)')
    ...
    close_parser.add_argument('symbol', type=str, help='Token symbol (e.g., DOGEUSDT)')
